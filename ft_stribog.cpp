...
fts::fts(const std::vector<unsigned char>& vec, std::size_t l_arity, std::string_view out_prefix) :
    l_arity_(l_arity),
    tau_(0),
    sblock_number_{0},
...
fts::fts(std::vector<unsigned char>&& vec, std::size_t l_arity, std::string_view out_prefix) :
    l_arity_(l_arity),
    tau_(0),
    sblock_number_{0},
    sblock_index_{0x80, 0x00},
...
std::vector<unsigned char> fts::ft()
{
    ...
    hash_512(&(*v_.begin()), v_.size(), &(*v_.begin()));
    this->v_.resize(64);
